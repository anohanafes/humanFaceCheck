...
2. **活体检测**：要求用户完成指定动作（张嘴、转头），防止照片欺骗
...
```
预设照片 → 提取特征向量 → 用户上传待验证照片 → 提取特征 → 比对 → 返回结果
```

1. **注册阶段**：从预设照片中提取 128 维人脸特征向量
2. **验证阶段**：用户上传待验证照片，提取特征后与注册特征比对
3. **结果判定**：单次比对，距离小于阈值（0.35）则通过

> 图片对比模式没有活体检测和连续帧验证，安全性较低，适用于低风险场景。
...
> 以下机制仅适用于**摄像头模式**，图片对比模式使用固定阈值 0.35。
